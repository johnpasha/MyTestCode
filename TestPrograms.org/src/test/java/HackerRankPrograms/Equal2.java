...
//		int[] array= new int[]{10,50,120,161};	
	...
	static int equal(int[] arr) {
		...
		if((max-last_Unmatched)/5>0 || max-min>=5) {
			multiplier = Math.max((max-last_Unmatched)/5,1);
			...
			return equal(arr)+multiplier;
			 
		}
		else if((max-last_Unmatched)/2>0 || max-min>=2) {
			multiplier = Math.max((max-last_Unmatched)/5,1);
			for(int i=0;i<arr.length-1;i++) {
				System.out.println(multiplier);
				arr[i]+=2*multiplier;
